...
export default function AdminProperties() {
  ...
  useEffect(() => { const load = async () => { setLoading(true); const { data } = await supabase.from("properties").select("*").order("created_at", { ascending: false }); setProperties(data || []); setLoading(false); }; load(); }, [supabase]);
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-10 bg-[#EDE8DF] flex-shrink-0 overflow-hidden">
                          {p.image && <Image src={p.image} alt="" fill style={{ objectFit: "cover" }} />}
                        </div>
                        <div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                ...
                <div>
                  ...
                  >
                    {imagePreview ? (
                      <>
                        <Image src={imagePreview} alt="" fill style={{ objectFit: "cover" }} />
